...
const Navbar = () => {
  ...
  return (
    <div className={`w-full flex   ${backgroundnav ? 'newnavbar ' : 'bg-gradient-to-r from-red-600 to-[#3762be]'} py-2 justify-between items-center navbar`}>
      <div className='w-[98%] mx-auto'>
        <Link to='/'>
        <p className='bg-red-500 rounded-lg text-white'>Goshop</p>
        </Link>
        ...
        <ul className='list-none sm:flex hidden  justify-end items-center  gap-3 mr-10 '>
          ...
          {user ? <button
            ...
            className='text-white bg-red-900 border-[#f5f8f613] border px-2 rounded-md py-0.5  '>Log Out</button> : <li className='text-white'><Link to='/login'>Sign In</Link></li>}
